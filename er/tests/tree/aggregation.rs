...
pub fn mixed() {
    ...
    let tree = er_all!(Batch::new, [success, plain, boxed, existing]).unwrap_err();
    ...
    assert_eq!(messages, ["Item(1)", "disk gone", "Item(2)"]);

    assert!(tree.er_contains::<io::Error>());
    assert_eq!(tree.nodes[2].nodes.len(), 1);
    assert_eq!(tree.nodes[2].nodes[0].er_find::<Item>().unwrap().0, 3);
    #[cfg(feature = "src_locations")]
    assert_eq!(
        (
