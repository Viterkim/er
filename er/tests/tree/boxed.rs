...
#[derive(Debug)]
pub struct DriverEr(pub io::Error);
impl fmt::Display for DriverEr {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str("driver rejected the request")
    }
}
impl Error for DriverEr {
...
pub fn ingestion() {
    ...
    let result: Result<(), Box<dyn Error + Send + Sync>> = Err(Box::new(DriverEr(cause)));
    let tree = result.er(StageEr::new).er(OuterEr::new).unwrap_err();
    let driver = tree.er_find::<DriverEr>().unwrap();
    let source = tree.er_find::<io::Error>().unwrap();

    assert!(core::ptr::eq(source, &driver.0));
    assert_eq!(source.kind(), io::ErrorKind::PermissionDenied);
    assert!(tree.er_contains::<OuterEr>());
    assert!(tree.er_contains::<StageEr>());
    ...
    assert_eq!(
        tree.er_descendants()
            ...
            .collect::<Vec<_>>(),
        ["StageEr", "driver rejected the request"]
    );
    ...
    let tree = result.er(StageEr::new).unwrap_err();
...
pub fn erased_nodes() {
    ...
    let cause = io::Error::new(io::ErrorKind::PermissionDenied, "no access");
    let boxed: BoxError = Box::new(DriverEr(cause));
    let tree = ErTree::new(OuterEr, [node, boxed.into_er_node()]);

    assert!(tree.er_contains::<StageEr>());
    assert!(tree.er_contains::<DriverEr>());
