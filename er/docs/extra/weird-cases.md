...
If a foreign trait needs your Wrap to implement `Error`, you can use `std_error`. Then normal `.er()` boxes the Wrap and `.er_find()` can't see inside it. Use `.er_wrap()` when the Wrap comes back. This is probably the worst bit of Er right now. [The example](../macros.md#wrap-with-std_error) shows it.
