...
Thinking about this more im not sure it would be beneficial because changing a function from returning A to B, then fixing the places that used A, is annoying. I'd rather give each function its own small `NameErr` from the start instead of exposing the inner type. And if you say "Try can do that as well" then yes ofcourse it can, but it become very very easy to then not add a new error to that spot, instead using the inner one which then gets error variants/cases it has no way of actualy doing, and then you end up with the thiserror 'mega error enums' that have nothing to do with the scope they are in.
